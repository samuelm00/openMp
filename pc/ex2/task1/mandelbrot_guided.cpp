...
bool mandelbrotGuidedCheck(complex<double> c, vector<int>& pixel) {
    ...
    for (const auto& nr: nrOfThreads) {
        ...
        for (i = 0; i < height; i++) {
            for (j = 0; j < width; j++) {
                ...
                if ( mandelbrotGuidedCheck(c, pixel) ) {
                    #pragma omp atomic
                    pixels_inside++;
                };
        ...
        // save image
        std::ofstream ofs("mandelbrot_guided.ppm", std::ofstream::out);
        ofs << "P3" << std::endl;
