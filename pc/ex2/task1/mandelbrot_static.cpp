...
#include <chrono>
#include <omp.h>
#include "mandelbrot_dynamic.cpp"
#include "mandelbrot_guided.cpp"
...
bool mandelbrotStaticCheck(complex<double> c, vector<int>& pixel) {
...
void mandelBrotStatic() {
    ...
    for (const auto& nr: nrOfThreads) {
        ...
        #pragma omp parallel for schedule(static) collapse(2) private(c) firstprivate(pixel, height, width)
        for (i = 0; i < height; i++) {
            for (j = 0; j < width; j++) {
                c = complex<double>( 2.0*((double)j/width-0.75), ((double)i/height-0.5)*2.0);

                if ( mandelbrotStaticCheck(c, pixel) ) {
                    #pragma omp atomic
                    pixels_inside++;
                };
        ...
        // save image
        std::ofstream ofs("mandelbrot_static.ppm", std::ofstream::out);
        ofs << "P3" << std::endl;
